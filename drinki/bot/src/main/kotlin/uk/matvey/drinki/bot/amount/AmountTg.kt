...
object AmountTg {
    ...
    fun amountsKeyboard(): InlineKeyboardMarkup {
        return InlineKeyboardMarkup(
            arrayOf("some",  "1/1oz", "2/1oz",  "3/1oz" ).map(Amount.Companion::parse).map { InlineKeyboardButton(it.label()).callbackData("/drink_ing_amt $it") }.toTypedArray(),
            arrayOf("1/4oz", "5/4oz", "9/4oz",  "13/4oz").map(Amount.Companion::parse).map { InlineKeyboardButton(it.label()).callbackData("/drink_ing_amt $it") }.toTypedArray(),
            arrayOf("1/3oz", "4/3oz", "7/3oz",  "10/3oz").map(Amount.Companion::parse).map { InlineKeyboardButton(it.label()).callbackData("/drink_ing_amt $it") }.toTypedArray(),
